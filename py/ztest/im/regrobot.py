...
def regrobot(uid,avatar,name,level):
	...
	try:
		...
		# print(json_params)
		conn.request("POST", "/add_user?debug=1&live_uid=1000&uuid="+str(robot["uid"]), json_params, tempHead)
...
def unregrobt(uid):
	try:
		...
		# print(json_params)
		conn.request("POST", "/del_user?debug=1&live_uid=1000&uuid="+str(uid), {}, tempHead)
		response = conn.getresponse()
		data = response.read()

	...
	with open(full_path,"rb") as plist_file:
		...
		for i in range(len(atlass)):
			...
			for j in range(atlas["start"]+5,atlas["end"],7):
				...
				print("key = ",k)
				#第一步取出图片在大图中大小位置
				rectlist = [item_x,item_y,item_w,item_h] #to_list(v['frame'])
				...
				print("box=",box)
				sizelist = [item_orig_w,item_orig_h]#[ int(x) for x in to_list(v['sourceSize']) ]
				...
				if item_rotate:
				    ...
				    img_rect_on_big = img_rect_on_big.rotate(-90,Image.NEAREST,True)
				...
				rect_offset = [item_offset_x,item_offset_y]#[ int(x) for x in to_list(v['offset']) ]
				...
				result_box_offset[0] = offset_x #+ int((item_orig_w-item_w)/2)
				result_box_offset[1] = offset_y #+ int((item_orig_h-item_h)/2)

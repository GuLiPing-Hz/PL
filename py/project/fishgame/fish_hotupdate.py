...
def creatLobbyManifest(ver_pre, version, url, src, dest, next=False):
    ...
    if(next):  # 是否生成下个版本的资源
        ...
        manifest["remoteManifestUrl"] = url_next + \
            "project.manifest"  # ,这里增加逗号，会表示这是一个数组
        ...
        file_helper.write_str_to_file(
            dest_dir+"/project.manifest", json.dumps(manifest, indent=0, sort_keys=False))

        del manifest["assets"]
        del manifest["searchPaths"]
        file_helper.write_str_to_file(
            dest_dir+"/version.manifest", json.dumps(manifest, indent=0, sort_keys=False))
...
def createGameManifest(game_id, url, ver, game_dir, src, dest, need_first=True):
    ...
    # return
    file_helper.write_str_to_file(
        game_manifest_dir+"/"+game_pro_manifest_name, json.dumps(manifest, indent=0, sort_keys=False))

    del manifest["assets"]
    del manifest["searchPaths"]
    file_helper.write_str_to_file(
        game_manifest_dir+"/"+game_ver_manifest_name, json.dumps(manifest, indent=0, sort_keys=False))
